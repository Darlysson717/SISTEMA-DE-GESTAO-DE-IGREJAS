-- Restore role column in profiles if it was dropped
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS role text not null default 'community' check (role in ('admin', 'volunteer', 'community'));

-- Recreate professional_profiles table if it was dropped
...
drop policy if exists "Community reads active professionals" on public.professional_profiles;
...
drop policy if exists "Professionals read own record" on public.professional_profiles;
create policy "Professionals read own record"
on public.professional_profiles
for select
using (auth.uid() = user_id);

-- Recreate professional_availabilities table
...
drop policy if exists "Everyone can read active availabilities" on public.professional_availabilities;
...
drop policy if exists "Professional manages own availabilities" on public.professional_availabilities;
...
drop policy if exists "Community reads own appointments" on public.appointments;
create policy "Community reads own appointments"
on public.appointments
for select
using (auth.uid() = community_user_id);

drop policy if exists "Professional reads own appointments" on public.appointments;
...
drop policy if exists "Community creates own appointments" on public.appointments;
...
drop policy if exists "Professional updates own appointments" on public.appointments;
